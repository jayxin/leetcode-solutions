...
// 评测系统将根据这些输入创建链式数据结构，并将两个头节点 headA 和 headB 传递给你的 程序。如果程序能够正确返回相交节点，那么你的解决方案将被 视作正确答案 。
//  
// 示例 1：
...
struct ListNode *getIntersectionNode(struct ListNode *headA, struct ListNode *headB) {
  // prevA: previous node of curA, prevB: previous node of curB
  // curA: current node of headA, curB: current node of headB
  struct ListNode *prevA, *prevB, *curA, *curB;
  // lA: length of headA, lB: length of headB, diff: difference of lA and lB
  int lA, lB, diff;
  ...
  // compute length and prev of headA
  while (curA) {
    ++lA;
    prevA = curA;
    curA = curA->next;
  ...
  // compute length and prev of headB
  while (curB) {
    ++lB;
    prevB = curB;
    curB = curB->next;
  ...
  // the length of headA and headB is not equal
  if (lA > lB) {
    diff = lA - lB;

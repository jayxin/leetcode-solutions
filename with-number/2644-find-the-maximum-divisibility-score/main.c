...
// 2644. 找出可整除性得分最大的整数 - 给你两个整数数组 nums 和 divisors 。
...
int maxDivScore(int* nums, int numsSize, int* divisors, int divisorsSize) {
  int cnts[divisorsSize];

  memset(cnts, 0, sizeof(cnts));

  for (int i = 0; i < divisorsSize; i++) {
    for (int j = 0; j < numsSize; j++) {
      if (nums[j] % divisors[i] == 0) ++cnts[i];
    }
  }

  int ret = divisors[0], c = cnts[0];

  for (int i = 1; i < divisorsSize; i++) {
    if (cnts[i] > c) {
      ret = divisors[i];
      c = cnts[i];
    } else if (cnts[i] == c && divisors[i] < ret) {
      ret = divisors[i];
    }
  }

...
// 539. 最小时 间差 - 给定一个 24 小时制（小时:分钟 "HH:MM"）的时间列表，找出列表中任 意两个时间的最小时间差并以分钟数表示。
//  
// 示例 1：
...
//int findMinDifference(char** timePoints, int timePointsSize) {
  //struct Pair p[timePointsSize];
  //int ret, limit = 24 * 60, d;
//
  //for (int i = 0; i < timePointsSize; i++) {
    //p[i].hour = (timePoints[i][0] & 0x0f) * 10 + (timePoints[i][1] & 0x0f);
    //p[i].min = (timePoints[i][3] & 0x0f) * 10 + (timePoints[i][4] & 0x0f);
    //p[i].tot = p[i].hour * 60 + p[i].min;
  //}
//
  //qsort(p, timePointsSize, sizeof(struct Pair), cmp);
//
  //ret = p[1].tot - p[0].tot;
//
  ...
  //}
//
  //d = limit - p[timePointsSize-1].tot + p[0].tot;
  //if (d < ret) ret = d;
//
  //return ret;
//}
